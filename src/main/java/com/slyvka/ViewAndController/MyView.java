...
//        methodsMenu.put("A", this::selectAllTable)
    ...
    private void findById() throws SQLException {
        ...
        switch (selectedTable) {
            ...
            case "money_transfer":
                id = Integer.parseInt(input.nextLine());
                System.out.println("Write landlord`s id");
                int landlordId = Integer.parseInt(input.nextLine());
                System.out.println("Write tenant`s id");
                int tenantId = Integer.parseInt(input.nextLine());
                System.out.println(new MoneyTransferService().findById(id, landlordId, tenantId));
                break;
        }
    }
    ...
    private void create() throws SQLException, ParseException {
        switch (selectedTable) {
            case "landlord":
                ...
                new LandlordService().create(new LandlordEntity(id, name, surname, BigDecimal.valueOf(money)));
                break;
            case "tenant":
                System.out.println("id");
                id = Integer.parseInt(input.nextLine());
                ...
                new TenantService().create(new TenantEntity(id, name, surname, BigDecimal.valueOf(money)));
                break;
            case "city":
                System.out.println("name");
                name = input.nextLine();
...
//                java.sql.Date timeSQL = java.sql.Date.valueOf(String.valueOf(timeUtil));
                ...
                int tenantID = Integer.parseInt(input.nextLine());
                new MoneyTransferService().create(new MoneyTransferEntity(id, time, BigDecimal.valueOf(money), convertBoolToByte(isSent), convertBoolToByte(mayBeReceived),
                        convertBoolToByte(isReceived), landlordID, tenantID));
                break;
        }
    }
    ...
    private void update() throws SQLException, ParseException {
        switch (selectedTable) {
            case "landlord":
                ...
                System.out.println(new LandlordService().update(new LandlordEntity(id, name, surname,
            ...
            case "tenant":
                ...
                System.out.println(new TenantService().update(new TenantEntity(id, name, surname,
                        BigDecimal.valueOf(money))));
                break;
            case "money_transfer":
                System.out.println("id to update");
                ...
                System.out.println("tenant_id");
                int tenantID = Integer.parseInt(input.nextLine());
                System.out.println(new MoneyTransferService().update(new MoneyTransferEntity(id, time,
                        BigDecimal.valueOf(money), convertBoolToByte(isSent), convertBoolToByte(mayBeReceived),
                        convertBoolToByte(isReceived), landlordID, tenantID)));
                break;
        }
    }
    ...
    private void delete() throws SQLException {
        ...
        switch (selectedTable) {
            ...
            case "money_transfer":
                ...
                System.out.println("Write tenant`s id");
                int tenantId = Integer.parseInt(input.nextLine());
                System.out.println(new MoneyTransferService().delete(id, landlordId, tenantId));
                break;
        }
    }
